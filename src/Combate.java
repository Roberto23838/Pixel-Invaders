    ...
    public void actualizarBalasYColisionarBalas() {
        ...
        for (Proyectil p : proyectiles) {
            if (p.isActivo()) {
                int x = p.getX();
                int y = p.getY();
                if (esCeldaValida(x, y) && (mapa[x][y] == '^' || mapa[x][y] == 'ˇ')) {
                    mapa[x][y] = '.';
                }
            }
        }
        ...
        for (Proyectil p : proyectiles) {
            ...
            if ((anterior == '^' && !p.esDeJugador()) || (anterior == 'ˇ' && p.esDeJugador())) {
                p.desactivar();
                balasInactivas.add(p);
                ...
                nuevoMapa[x][y] = p.esDeJugador() ? '^' : 'ˇ';
            }
        }

        for (int i = 0; i < filas; i++) {
            for (int j = 0; j < columnas; j++) {
                if (nuevoMapa[i][j] == '^' || nuevoMapa[i][j] == 'ˇ') {
